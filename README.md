...
| LLM — zapytania | Groq `llama-3.1-8b-instant` / Ollama `llama3.1` |
| LLM — synteza | Groq `llama-3.3-70b-versatile` |
...
3. Retrieval hybrydowy (wektorowy + pełnotekstowy) z rerankingiem
4. Synteza odpowiedzi przez LLM (`llama-3.3-70b-versatile`)
5. Zwraca `summary_text` + `key_numbers` + `source_data` (filename + page_ref + node_content)
...
- Repozytorium ignoruje lokalne artefakty (`files/`, `uploads/`, `data/`) oraz sekrety (`.env`)
- Klucze API należy uzupełnić samodzielnie w `.env`

---

## Ewaluacja

<!-- Miejsce na notatki z ewaluacji -->